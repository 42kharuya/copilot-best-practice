...
「**物理ルール（手順・環境）**」と「**対話ルール（振る舞い・コード品質）**」を分けて管理するのがポイントです。
...
| **物理ルール<br>(全体)** | `AGENTS.md`<br>*(ルート直下)* | **プロジェクト全体の開発ガイド**<br>ビルド手順、テスト実行コマンド、環境構築など。他のAIツール（Cursor等）とも共有される標準規格。 | AIが最初に「このプロジェクトはどう動かすの？」と迷わないようにするため。 |
| **物理ルール<br>(局所)** | `各ディレクトリ/AGENTS.md` | **特定フォルダ内の開発ガイド**<br>そのディレクトリ特有のビルド手順や依存関係の扱いなど。ディレクトリごとに役割が全く違う場合（フロントとバックなど）に利用。 | フォルダ単位で実行するコマンドやアーキテクチャの前提が大きく異なる場合。 |
